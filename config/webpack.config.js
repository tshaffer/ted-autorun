...
module.exports = function (webpackEnv) {
  ...
  const externals = [];
  if (isEnvProduction && isStandalone) {
    externals.push({
  ...
  const shouldUseReactRefresh = envVar.raw.FAST_REFRESH;
  ...
  return {
    ...
    output: {
      libraryTarget: 'umd',
      // The build folder.
      path: isEnvProduction ? distDeploymentDir : undefined,
      // Add /* filename */ comments to generated require()s in the output.
      pathinfo: isEnvDevelopment,
      // There will be one main bundle, and one file per asynchronous chunk.
      // In development, it does not produce real files.
      filename: isEnvProduction
        ? '[name]' +  (isStandalone ? '.[contenthash:8]' : '.autorun') + '.js'
    ...
    optimization: {
      ...
      splitChunks: (isEnvDevelopment || isStandalone) && {
        chunks: 'all',
        name: false,
      },
      ...
      runtimeChunk: (isEnvDevelopment || isStandalone) && {
    ...
    plugins: [
      ...
      (isEnvDevelopment || isStandalone) && new InterpolateHtmlPlugin(HtmlWebpackPlugin, envVar.raw),
